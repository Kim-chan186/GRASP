...
def train(epoch, anchornet: nn.Module, localnet: nn.Module,
    ...
    for anchor_data, rgbs, rgb2, depths, grasppaths in tqdm(train_data,
        ...
        #?@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
        #? AnchorNet
        #! output : (loc_map=heatmap, cls_mask, th_offset, d_offset, w offset), features
        x, y, _, _, _ = anchor_data
        ...
        anchor_lossd = compute_anchor_loss(pred_2d,
                                           target,
                                           loc_a=args.loc_a, # = 1
                                           reg_b=args.reg_b, # = 5 #TODO 어디서 5로 바꾼건지 모름
                                           cls_c=args.cls_c) # = 1
        #! losses : loc, reg, cls
        #! loss : loss = loc+reg+cls
        anchor_losses = anchor_lossd['losses']
        anchor_loss = anchor_lossd['loss']
        ...
        if epoch >= args.pre_epochs:
            ...
            for i in range(args.batch_size):
                rect_gg = detect_2d_grasp(loc_maps[i],
                                          ...
                                          width_offset[i],
                                          ratio=args.ratio,          # = 8
                                          anchor_k=args.anchor_k,    # = 6
                                          anchor_w=args.anchor_w,    # = 50
                                          anchor_z=args.anchor_z,    # = 20
                                          mask_thre=0,
                                          center_num=args.center_num,# = 128
                                          grid_size=args.grid_size,  # = 8
                                          grasp_nms=args.grid_size)  # = 8
